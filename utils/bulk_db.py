...
from restaurants.models import MainCategory, Restaurant, Menu
import math
import random
import requests
import json
...
def gen_restaurant_data(request: HttpRequest):
    ...
    for main_info in main_info_content:
        ...
        restaurant_database.update({
            'main_category': main_info['main_category'],
            'sub_category': sub_info_content['sub_category'],
            'near_station': main_info['near_station'],
            'name': main_info['name'],
            'address': sub_info_content['address'],
            'latitude': latitude,
            'longitude': longitude,
            'distance': distance
        })
...
# generate menu data
def gen_menu_data(request: HttpRequest):
    header = {
        'authority': 'map.naver.com',
        'method': 'GET',
        'scheme': 'https',
    ...
    for restaurant in Restaurant.objects.all():
        ...
        try:
            ...
            content_json = json.loads(response.content)['result']['place']['list'][0]
            time.sleep(2)
            menu_info = content_json['menuInfo']
            menu_price_sets = menu_info.split(' | ')
            
            cnt = 0
            for menu_price_set in menu_price_sets:
                menu_database = {
                    'restaurant': restaurant,
                    'name': None,
                    'price': None
                }
                menu_price_set = menu_price_set.rsplit(' ', 1)
                menu_name = menu_price_set[0]
                menu_price = int(menu_price_set[1].replace(',', ''))
                menu_database.update({
                    'name': menu_name,
                    'price': menu_price
                })
                menu_databases.append(menu_database)
                cnt += 1
        except:
            if cnt == 0:
                menu_databases.append({
                        'restaurant': restaurant,
                        'name': '메뉴없음',
                        'price': 0
                    })
    
    Menu.objects.bulk_create([Menu(**data) for data in menu_databases], ignore_conflicts=True)
    return HttpResponse(status=200)